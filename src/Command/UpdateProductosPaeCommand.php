...
/**
 * Class UpdateProductosPaeCommand
 * @package App\Command
 */
class UpdateProductosPaeCommand extends Command
{
    ...
    {
        $this->setName('gsbase:update:productospae')
            ->setDescription('Comando que actualiza la entidad ProductosPae');
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
     ...
     if ($lastLine != 'SI') {
        ...
        $operatorsProductosPae = $em->getRepository(Operator::class)->findBy(array(
            'opEntity' => 'ProductosPae', 'opEst' => 'C'));
        ...
        foreach ($operatorsProductosPae as $operator) {
            ...
            $registersProcessed = $registersProcessed + $ProductosPaeOperator['registersProcessed'];
            $registersCreated = $registersCreated + $ProductosPaeOperator['registersCreated'];
            $registersUpdated = $registersUpdated + $ProductosPaeOperator['registersUpdated'];    
        }
        ...
        fwrite($log, ("PRODUCTOSPAE => Comienzo: " . $updateStart . " | Final: " . $updateEnd .
