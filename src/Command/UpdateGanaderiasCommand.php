...
class UpdateGanaderiasCommand extends Command
{
    ...
    {
        $this->setName('gsbase:update:ganaderias')
            ->setDescription('Comando que actualiza la entidad Ganaderias');
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $operatorsGanaderia = $em->getRepository(Operator::class)->findBy(
        ...
        foreach ($operatorsGanaderia as $operator) {
            $ganaderiasOperator = $toolsupdate->getGanaderiasOperator($gsbase, $gsbasexml, $operator);
            $registersProcessed = $registersProcessed + $ganaderiasOperator['registersProcessed'];
            $registersCreated = $registersCreated + $ganaderiasOperator['registersCreated'];
            $registersUpdated = $registersUpdated + $ganaderiasOperator['registersUpdated'];
        }
        ...
        fwrite(
            $log,
            ("GANADERIAS => Comienzo: " . $updateStart . " | Final: " . $updateEnd . " | Registros Procesados: " .
