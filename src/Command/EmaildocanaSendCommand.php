...
class EmaildocanaSendCommand extends Command
{
    ...
    {
        $this->setName('email:emaildocana:send')
...
EOF
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $path_file = $urlBase.'register_recorridos_ANA_'.date("d_m_Y").'.log';
                ...
                if ((strpos($lista[$i], '.pdf') !== false) && (strpos($lista[$i], 'Untitled') === false) && (strpos($lista[$i], '-') !== false)) {
                    ...
                    /*if (isset($datamail)) {
                        ...
                        if($datamail["mail"] != null){
                            ...
                            //(var_dump("valor de send()---->".$mailer->send($message));
                            if($mailer->send($message) != 1){
                                if ($operador["opEma"]!=''){
                                    if (isset($operador["opCdp"])) {
                                        $docNew->setOpCdp($operador["opCdp"]);
        ...
        # Definimos la Ruta Completa y el Nombre del Fichero LOG que se va a generar
        $path_file = $urlBase.'update_datedocuments_ANALISIS_'.date("d_m_Y").'.log';
        ...
        fwrite(
            $log,
            ("\n - Analisis => Total: ". $totalCert ." | Procesados: ". $procCert ." | Nuevos: ". $cerNew ." | Actualizados: ". $cerUpdate ." | Sin Operador: ". $cerSO ."\n")
        );

        # Escribimos Información sobre Documentos No Válidos
        fwrite(
            $log,
            ("\n - Documentos NO Válidos => Analisis: ". $docNVFact ." | Analisis: " . $docNVCert ." | Analisis: " . $docNVCart ." | Analisis: " . $docNVAna . "\n")
        );
    ...
    private function createMessage(InputInterface $input, Array $datos)
    {
        ...
        $to =$destino; //'maria.gonzalez@atlantic.es';
                                        ...
                                        <tr>
                                            <td valign="top" class="body-cell">
                                                <table cellpadding="0" cellspacing="0" width="100%" bgcolor="#ffffff">
                                                    ...
                                                    <tr>
                                                        <td>
                                                            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;- Denominación Cliente: <b>' . $nombre . '</b>
                                                        </td>
                                                    </tr>
                                                    <tr>
                                                        <td><br>&nbsp;<br></td>
                                                    ...
                                                    <tr>
                                                        <td align="center">
                                                            <br>
                                                            <br>
                                                            <p><a target="_blank" href="http://clientes.sohiscert.com/login" style="text-decoration:none"><font size="3px" face="arial" color="#40733c" weight="500"><b> ACCEDER AL ÁREA PRIVADA DE CLIENTES</b></font></a>
                                                            </p>
                                                        </td>
                                                    </tr>
                                                </table>
